...
/**************************************************************************/
void childInterface::
Initialize( string argument_string, int nox, int noy, double*** pts )
{
  ...
  if( !option.silent_mode )
    std::cout << "Creating mesh...\n";
  //mesh = new tMesh<tLNode>( inputFile, option.checkMeshConsistency );
  mesh = new tMesh<tLNode>(inputFile, nox, noy, pts, option.checkMeshConsistency);
...
/**************************************************************************/
void childInterface::
Initialize( int argc, char **argv, int nox, int noy, double*** pts )
{
  string argstr;
  for( int i=1; i<argc; i++ )
  {
...
Run( double run_duration )
{
	...
	std:cerr <<time->getCurrentTime()<<" Remaining: "<<time->RemainingTime()<<std::endl;
		RunOneStorm();
	}
...
ModifyCoords( double*** newCoord )
{
   ...
   {
	   double coordx = (reinterpret_cast<double *>((reinterpret_cast<double **>(newCoord))[cn->getPermID()]))[0];
	   double coordy = (reinterpret_cast<double *>((reinterpret_cast<double **>(newCoord))[cn->getPermID()]))[2];
	   ...
	   cn->setZ( (reinterpret_cast<double *>((reinterpret_cast<double **>(newCoord))[cn->getPermID()]))[1] );
...
GetNewCoords( double*** newCoords )
{
   ...
   for( cn=ni.FirstP(); !ni.AtEnd(); cn=ni.NextP() )
   {
	   ...
	   // Note that SNAC's y is vertical (z in CHILD).
	   //(reinterpret_cast<double *>(reinterpret_cast<double **>(newCoords)[cn->getPermID()]))[0] = cn->getX();
